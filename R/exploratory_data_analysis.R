...
explore_storm_data <- function(storm_data) {
  ...
  storm_counts <- dplyr::count(storm_data, storm_data$magnitude_type )

  # Calculate mean duration for each storm category
  mean_durations <- storm_data %>%
    dplyr::group_by(storm_data$magnitude_type ) %>%
    dplyr::summarize(mean_duration_of_storm = mean(storm_data$storm_duration, na.rm = TRUE))

  # Calculate event type counts
  event_type_counts <- dplyr::count(storm_data, storm_data$event_type)

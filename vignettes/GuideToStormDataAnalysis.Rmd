...
The capabilities offered in this package can definitely be fulfilled with other standard R packages. But the following package is motivated to deal specifically with Storm Dataset and fused Storm Datasets. Additionally, related weather or air pollution data can be combined with the results obtained to make suitable data-driven decisions in the field of Environmental Study, Meteorology or Geo-Spatial Data Analysis and Predictions.
...
We will now add new columns to the dataset using the add_new_feature function defined in the data_feature_engineering.R script:
...
For result analysis, we are considering ggplot2 library along with our StormDataAnalysis Package. The plots helps the user visualize the data trends with better clarity.
...
#### Issues with R CMD check:

I have received 3 notes on executing R CMD check.

```
❯ checking installed package size ... NOTE
    installed size is 67.4Mb
```
I believe the package size is less than 80 MB along with the datasets. 
So there is not an urgent requirement for package size optimization.
...
```
I am using ggplot2 for obtaining plot visualizations in my vignette.
Although, it is not used while writing the functions in my package, but importing the package makes it more convenient, and I do not need to import the package while executing my vignette.

```
❯ checking R code for possible problems ... NOTE
  clean_data: no visible binding for global variable 'tor_f_scale'
  clean_data: no visible binding for global variable 'tor_length'
  clean_data: no visible binding for global variable 'tor_width'
  clean_data: no visible binding for global variable 'tor_other_wfo'
  explore_storm_data: no visible binding for global variable
    'magnitude_type'
  explore_storm_data: no visible binding for global variable
    'storm_duration'
  explore_storm_data: no visible binding for global variable 'event_type'
  explore_storm_data: no visible binding for global variable
    'mean_duration_of_storm'
  Undefined global functions or variables:
    event_type magnitude_type mean_duration_of_storm storm_duration
    tor_f_scale tor_length tor_other_wfo tor_width
```
All the following above are names of columns in a dataframe. But since I am using the dplyr library, it is not mandatory to reference them with a '$' operator.

For example, I can write `data$magnitude_type`; and this Note will be removed in the next CMD check. But at times, my code is giving unexpected output. It seems to be a problem with roxygen.
